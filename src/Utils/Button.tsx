...
const Button = ({ variant, label, onPress, textTransform }: ButtonProps) => {
    ...
    const backgroundColor = variant === "primary" ? theme.colors.button : theme.colors.grey;
    ...
    return (
        <RectButton {...{ onPress }} style={[styles.container, { backgroundColor }]}>
            <Text style={{ color }} textTransform={textTransform ? textTransform : "lowercase"} variant="button">
                {label}
            </Text>
        </RectButton>
    )
